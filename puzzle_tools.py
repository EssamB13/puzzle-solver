...
# k so i think i got this. the queue makes sense to me. still have the same
# problem as depth_first search, but
# if we solve it for one we've solved it for both.


def create_puzzle_node(puzzle_path):
    """
    Create a PuzzleNode that contains a path to the solution
    @type puzzle: list[Puzzle]
    @type _puzzle_node: PuzzleNode
    @return: PuzzleNode
    """
